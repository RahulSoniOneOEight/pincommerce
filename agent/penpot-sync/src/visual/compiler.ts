...
export function compileVisualRenderCode(target: VisualRenderTarget, remoteRootId: string): string {
  ...
  return `const repoId=${repoId}; const remoteRootId=${remote}; const prefix=${js(prefix)}; const tree=${tree};\n` +
    ...
    `else if(node.type==='icon'){s=penpot.createRectangle(); s.resize(node.width,node.height); s.fills=[{fillColor:node.color||'#173B72',fillOpacity:node.opacity??0.14}]; s.borderRadius=Math.min(node.width,node.height)/4; const g=penpot.createText(({plumbing:'↔',electrical:'⚡',sanitary:'◫','paints-construction':'▧','hardware-tools':'⌁',agriculture:'⌁',search:'⌕',utility:'•••',delivery:'→','product-placeholder':'◆'}[node.recipe])||'◇'); if(g){g.fontSize=String(Math.max(12,Math.round(node.height*0.42))); g.fills=[{fillColor:node.color||'#173B72',fillOpacity:Math.min(1,(node.opacity??1)+0.35)}]; g.x=x+Math.max(4,node.width*0.28); g.y=y+Math.max(3,node.height*0.20); s.appendChild(g);}}\n` +
